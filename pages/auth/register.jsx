...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    console.log('Request Data:', formData); // Log request data to the console
    const data = await register_me(formData);
    if (data.success) {
      toast.success(data.message);
      setTimeout(() => {
        router.push('/auth/login');
      }, 1000); // Reduced timeout duration to 1 second
    } else {
        ...
        <div className="flex flex-col text-center items-center justify-center px-6 py-8 mx-auto h-screen lg:py-0 shadow-xl">
          {!showForm ? (
            <div className="flex space-x-4 mb-6">
              <div 
                className="w-1/2 p-8 rounded-lg cursor-pointer bg-white shadow-lg transition-transform transform hover:scale-105"
                onClick={() => handleRoleSelection('Applicant')}
              >
                <div className="flex flex-col items-center">
                  <FaUser className="text-6xl mb-4 text-indigo-600" />
                  <h2 className="text-xl font-bold">Register as Applicant</h2>
                  <p className="text-gray-500">Join us to apply for jobs.</p>
                </div>
              </div>
              <div 
                className="w-1/2 p-8 rounded-lg cursor-pointer bg-white shadow-lg transition-transform transform hover:scale-105"
              ...
              >
                <div className="flex flex-col items-center">
                  <FaUserTie className="text-6xl mb-4 text-indigo-600" />
                  <h2 className="text-xl font-bold">Register as Employer</h2>
                  <p className="text-gray-500">Join us to post jobs.</p>
                </div>
              </div>
            </div>
          ) : (
